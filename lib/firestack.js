...
export default class Firestack extends Singleton {
  ...
  constructor(options: Object = {}) {
    ...
    if (instance.options.errorOnMissingPlayServices && !this.googleApiAvailability.isAvailable) {
      let error: Object = new Error(`Google Play Services is required to run this application but no valid installation was found (Code ${this.googleApiAvailability.status}).`);
      error.code = this.googleApiAvailability.status;
      throw error;
    }
  }
  ...
  static doPlayServicesCheck(): Promise<boolean> {
    return promisify('doPlayServicesCheck', FirestackModule)();
  }
